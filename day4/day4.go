...
func main() {
	...
	for scanner.Scan() {
		...
		if line_str != "" {
			...
			if elf_1_low <= elf_2_low && elf_1_high >= elf_2_high { // elf_2 is contained within elf_1
				duplicate_counter++
			} else if elf_1_low >= elf_2_low && elf_1_high <= elf_2_high { // elf_1 is contained within elf_2
				duplicate_counter++
			}
		}
	}
